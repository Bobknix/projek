...
void rollinam () {
    ...
    if (win) {
        ...
        PlaySound(TEXT("skaityk.wav"), NULL, SND_FILENAME | SND_ASYNC);
    } else {
...
int main()
{
    ...
    while (playAgain) {
        if (bilietai==4 || bilietai==2) {
            PlaySound(TEXT("rekia.wav"), NULL, SND_FILENAME | SND_ASYNC);
        }
        if (bilietai == 7 || bilietai ==3) {
            PlaySound(TEXT("patalokas.wav"), NULL, SND_FILENAME | SND_ASYNC);
        }
        if (bilietai>0) {
    ...
    cout<<"dar zhaisi? jei normalus, spausk 1. jei trioxala, spausk 0"<<endl<<"liko "<<bilietai<<" talonu"<<endl<<"nepamirshk kad 94.21% saves negerbianchiu gambleriu isheina priesh laimedami aukso poda"<<endl;
    cin>>playAgain;
    bilietai--;
    } else {
        cout<<"wsio laiswas sasalink ish chia"<<endl;
    }
    }
    return 0;
